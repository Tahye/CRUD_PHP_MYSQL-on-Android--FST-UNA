        ...
        button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(TextUtils.isEmpty(Uid)){
                    uid.setError("UID S.V.P");
                    valid = false;
                }else {
                    valid = true;

                    if(TextUtils.isEmpty(Name)){
                        name.setError("Name S.V.P");
                        valid = false;
                    }else {
                        valid = true;

                        if(TextUtils.isEmpty(Address)){
                            address.setError("Address S.V.P");
                            valid = false;
                        }else {
                            valid = true;

                            if(TextUtils.isEmpty(Phone)){
                                phone.setError("Phone S.V.P");
                                valid = false;
                            }else {
                                valid = true;
                            }
                ...
                if(valid){
                    ...
                    StringRequest stringRequest = new StringRequest(Request.Method.POST, Constants.URL_ADD, new Response.Listener<String>() {
                        ...
                        public void onErrorResponse(VolleyError error) {
                            progressDialog.hide();
                            Toast.makeText(MainActivity.this, "Échec des données",Toast.LENGTH_SHORT).show();
                        }
                    })
                    {
                        // les données à insrées
